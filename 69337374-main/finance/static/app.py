...
def index():
    ...
    COLUMNS = {"Symbol": 0, "Name": 0, "Shares": 2, "Price": 2, "TOTAL": 2}
    ...
    data = {
        ...
        "usd_fields": list([k.lower() for k in COLUMN_NAMES[-2:]]),
        "cash": cash,
        "total": total
    }
...
@login_required
def diff():
    """View difference in stock prices"""
    # transactions records the stock price at the time of transaction
    # `index()` looks up current stock price to give an accurate representation
    # combine the two and display the difference in stock price to help the client
    # + add
    COLUMNS = {
        "Symbol": 0,
        "Name": 0,
...
def get_assets(user_id):
    ...
    for t in transactions:
        ...
        if symbol not in assets:
            assets[symbol] = copy(SYMBOL_INFO_DEFAULT)
            assets[symbol]["symbol"] = symbol
        assets[symbol]["shares"] += t["shares"]
        assets[symbol]["total"] += t["total"]

    # Add price and total
    for symbol in assets:
        info = assets[symbol]

        # Delete zero share symbols
        if info["shares"] == 0:
            del assets[symbol]
